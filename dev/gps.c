...
int gps_init()
{
  ...
  if(tty > 0)
  {
      fprintf(stderr,"closin\n");
      close(tty);
  }
  fprintf(stderr,"INIT\n");

  //system("stty -F /dev/gps 4800");
  tty = open("/dev/cu.PL2303-000013FD", O_RDWR | O_NOCTTY);
  fprintf(stderr,"INIT\n");
  if(tty < 0)
...
{
    unsigned char buf[128]; //Fill up with each line
    unsigned char latitude[12];
    unsigned char longitude[13];
    int read_retries = 0;
    int data_retries = 0;

	fprintf(stderr,"Getting Position from tty: %d\n",tty);

    latitude[11] = 0;
    longitude[12] = 0;
    
	if (read(0,&buf[0],128) <= 2)
	{
    	//Disregard data until $GPRMC is found and is valid
		//tcflush(tty, TCIFLUSH);
		//usleep(150000);
		//fprintf(stderr,"TCIFLUSH'd\n");
    	while(1)
    	{
			fprintf(stderr,"Reading from GPS.\n");
    	  	while (read(tty, &buf[0], 128) <= 0)
    	  	{
    	    	usleep(5000);
       	 		if(read_retries++ > 50)
...
main()
{
	...
	fprintf(stderr,"Input destination NMEA String:\n");
    if (gps_get_position(&dest) != GPS_NO_ERROR)
	{
        printf("Data inValid.\n");
		return 0;
	}
	fprintf(stderr,"Input current position NMEA String:\n");
	while(1)
	{
    if (gps_get_position(&pos) != GPS_NO_ERROR)
	{
        printf("Data inValid.\n");
		return 0;
	}

    printf("From %14f,%14f to %14f,%14f: %14f  at heading %14f\n",pos.latitude,pos.longitude,dest.latitude,dest.longitude,calc_target_distance(&pos,&dest),calc_target_heading(&pos,&dest));
	}
}
